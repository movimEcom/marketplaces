...
def main() -> None:
    ...
    no_data_count = sum(1 for i in report.items if i.score is None)
    regular_count = sum(1 for i in report.items if i.kind == "regular" and i.score is not None)
    ...
    print(f"\n{report.total} publicaciones clasificadas "
          ...
          f"{no_data_count} sin datos -> Crítico):")
    ...
    if report.skip_reasons:
        print("\nMotivo de las publicaciones sin datos (código HTTP de /item/{id}/performance):")
